...
uint32_t compute_headers_len(struct aws_array_list *headers) {
    if (!headers || !aws_array_list_length(headers)) return 0;
    ...
    for (size_t i = 0; i < headers_count; ++i) {
        ...
        headers_len += sizeof(header->header_name_len) +
                       header->header_name_len + 1;
...
int aws_event_stream_message_init(struct aws_event_stream_message *message, struct aws_allocator *alloc,
    ...
    if (message->message_buffer) {
        ...
        uint32_t running_crc = aws_checksums_crc32(message->message_buffer,
                                                   (int) (buffer_offset - message->message_buffer), 0);
        ...
        running_crc = aws_checksums_crc32(message_crc_boundary_start,
                                          (int) (buffer_offset - message_crc_boundary_start), running_crc);
        aws_write_u32(buffer_offset, running_crc);
...
int aws_event_stream_message_from_buffer(struct aws_event_stream_message *message,
                                         struct aws_allocator *alloc, struct aws_byte_buf *buffer) {
    assert(buffer);
    ...
    running_crc = aws_checksums_crc32(buffer->buffer + PRELUDE_CRC_OFFSET,
...
int aws_event_stream_message_headers(const struct aws_event_stream_message *message, struct aws_array_list *list) {
    return get_headers_from_buffer(list,
...
uint32_t aws_event_stream_message_payload_len(const struct aws_event_stream_message *message) {
    return aws_event_stream_message_total_length(message) -
           (AWS_EVENT_STREAM_PRELUDE_LENGTH +
            aws_event_stream_message_headers_len(message) + AWS_EVENT_STREAM_TRAILER_LENGTH);
}

uint32_t aws_event_stream_message_message_crc(const struct aws_event_stream_message *message) {
    return aws_read_u32(message->message_buffer +
...
int aws_event_stream_add_string_header(struct aws_array_list *headers, const char *name, uint8_t name_len,
...
int
aws_event_stream_add_int16_header(struct aws_array_list *headers, const char *name, uint8_t name_len, int16_t value) {
    struct aws_event_stream_header_value_pair header = {
            .header_name_len = name_len,
...
int
aws_event_stream_add_int32_header(struct aws_array_list *headers, const char *name, uint8_t name_len, int32_t value) {
    struct aws_event_stream_header_value_pair header = {
            .header_name_len = name_len,
            .header_value_len = sizeof(value),
            .value_owned = 0,
...
int aws_event_stream_add_bytebuf_header(struct aws_array_list *headers, const char *name, uint8_t name_len,
                                        uint8_t *value, uint16_t value_len, int8_t copy) {
    struct aws_event_stream_header_value_pair header = {
            .header_name_len = name_len,
            .header_value_len = value_len,
...
int aws_event_stream_add_uuid_header(struct aws_array_list *headers, const char *name, uint8_t name_len,
                                     const uint8_t *value) {
    struct aws_event_stream_header_value_pair header = {
            .header_name_len = name_len,
            .header_value_len = 16,
...
static int read_header_value(struct aws_event_stream_streaming_decoder *decoder, uint8_t *data, size_t len,
    ...
    if (!length_read) {
        /* save an allocation, this can only happen if the data we were handed is larger than the length of the header value.
         * we don't really need to handle offsets in this case. This expects the user is living by the contract that they
        ...
        /* a possible optimization later would be to only allocate this once, and then keep reusing the same buffer. for subsequent messages.*/
        if (current_header->header_value_type == AWS_EVENT_STREAM_HEADER_BYTE_BUF ||
            ...
            current_header->header_value.variable_len_val = aws_mem_acquire(decoder->alloc,
                                                                                    decoder->current_header.header_value_len);
...
static int read_header_name(struct aws_event_stream_streaming_decoder *decoder, uint8_t *data, size_t len,
    ...
    size_t max_read =
            len >= decoder->current_header.header_name_len - length_read ? decoder->current_header.header_name_len -
                                                                           length_read : len;
    ...
    if (decoder->message_pos == decoder->prelude.total_len) {
        ...
        if (message_crc == decoder->running_crc) {
            ...
            snprintf(error_message, sizeof(error_message),
                     "CRC Mismatch. message_crc was 0x08%" PRIX32 ", but computed 0x08%" PRIX32, message_crc,
                     decoder->running_crc);
            aws_raise_error(AWS_ERROR_EVENT_STREAM_MESSAGE_CHECKSUM_FAILURE);
            decoder->on_error(decoder, &decoder->prelude,
                              AWS_ERROR_EVENT_STREAM_MESSAGE_CHECKSUM_FAILURE, error_message, decoder->user_context);
            return AWS_OP_ERR;
        }
    }
...
static int payload_state(struct aws_event_stream_streaming_decoder *decoder, uint8_t *data,
    ...
    if (decoder->message_pos < decoder->prelude.total_len - AWS_EVENT_STREAM_TRAILER_LENGTH) {
        ...
        size_t segment_length = len > remaining_amount ? remaining_amount : len;
        int8_t final_segment = (segment_length + decoder->message_pos) ==
                               (decoder->prelude.total_len - AWS_EVENT_STREAM_TRAILER_LENGTH);
...
static int verify_prelude_state(struct aws_event_stream_streaming_decoder *decoder, uint8_t *data, size_t len,
    ...
    if (AWS_LIKELY(decoder->running_crc == decoder->prelude.prelude_crc)) {
        ...
        if (AWS_UNLIKELY(decoder->prelude.headers_len > MAX_HEADERS_SIZE ||
                                 decoder->prelude.total_len > MAX_MESSAGE_SIZE)) {
            aws_raise_error(AWS_ERROR_EVENT_STREAM_MESSAGE_FIELD_SIZE_EXCEEDED);
            char error_message[] = "Maximum message field size exceeded";

            decoder->on_error(decoder, &decoder->prelude,
                              AWS_ERROR_EVENT_STREAM_MESSAGE_FIELD_SIZE_EXCEEDED, error_message, decoder->user_context);
            return AWS_OP_ERR;
        }
        ...
        snprintf(error_message, sizeof(error_message),
                 ...
                 decoder->prelude.prelude_crc, decoder->running_crc);

        aws_raise_error(AWS_ERROR_EVENT_STREAM_PRELUDE_CHECKSUM_FAILURE);
        decoder->on_error(decoder, &decoder->prelude,
                          AWS_ERROR_EVENT_STREAM_PRELUDE_CHECKSUM_FAILURE, error_message, decoder->user_context);
        return AWS_OP_ERR;
    }
...
static int start_state(struct aws_event_stream_streaming_decoder *decoder,
    ...
    if (decoder->message_pos < AWS_EVENT_STREAM_PRELUDE_LENGTH) {
        if (len >= AWS_EVENT_STREAM_PRELUDE_LENGTH - decoder->message_pos) {
            memcpy(decoder->working_buffer + decoder->message_pos, data,
...
int aws_event_stream_streaming_decoder_pump(struct aws_event_stream_streaming_decoder *decoder,
